"""Application entrypoint and orchestration boundary."""
...
from .errors import NewsWithWillyError
...
def command_run(args: argparse.Namespace, settings: Settings) -> int:
    notifier = TelegramNotifier(
        bot_token="" if args.dry_run else settings.telegram_bot_token,
        chat_id="" if args.dry_run else settings.telegram_chat_id,
    )
...
def command_analyze(args: argparse.Namespace, settings: Settings) -> int:
    ...
    result = Orchestrator().analyzer.analyze_event(event)
...
def run(settings: Settings) -> Orchestrator:
    """Create and start the complete monitoring pipeline."""
    logger.info("%s started with %d keywords", settings.app_name, len(settings.keywords))
    logger.info("Impact alerts enabled at score >= %d", settings.impact_threshold)
    orchestrator = Orchestrator()
    orchestrator.start()
    return orchestrator
...
def main(argv: list[str] | None = None) -> int:
    """CLI entrypoint with user-safe handling for expected failures."""
    ...
    except NewsWithWillyError as exc:
