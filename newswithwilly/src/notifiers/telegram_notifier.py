...
class TelegramNotifier:
    ...
    def format_calendar_alert_message(analysis: AnalysisResult, event: NewsEvent) -> str:
        """Format a calendar preparation or post-release alert."""
        ...
        source = f"[Open source]({TelegramNotifier._escape_url(event.url)})" if event.url else TelegramNotifier._escape("Source: " + event.source)
        phase = "POST\\-RELEASE UPDATE" if calendar.actual else "PRE\\-RELEASE PLAN"
        countdown = "RELEASED" if calendar.actual else TelegramNotifier._escape(f"T-{minutes} MIN")
        caution = "Confirm the actual result and price reaction before acting\\." if calendar.actual else "Prepare, but wait for the actual release and price confirmation\\."
        actual_line = TelegramNotifier._escape(calendar.actual or "Not released")
        reasoning = TelegramNotifier._escape(analysis.reasoning)
        if impact == "HIGH":
            return (
                f"🚨 *WATCH OUT: USD HIGH\\-IMPACT EVENT*\n"
                ...
                f"🎯 *ACTION:* {TelegramNotifier._escape(analysis.action)}\n"
                f"{reasoning}\n\n"
                f"⚠️ {caution}\n\n"
                f"{source}"
            )
        return (
            f"🟠 *WATCH OUT: USD MEDIUM\\-IMPACT EVENT*\n"
            f"🇺🇸 {TelegramNotifier._escape(event.headline)} \\| MEDIUM\n"
            f"⏰ Release: {TelegramNotifier._escape(release_time.strftime('%H:%M EAT'))} \\| {countdown}\n\n"
            f"📊 {phase}\n"
            f"Previous: {TelegramNotifier._escape(calendar.previous or 'Unavailable')}\n"
            f"Forecast: {TelegramNotifier._escape(calendar.forecast or 'Unavailable')}\n\n"
            f"Actual: {actual_line}\n\n"
